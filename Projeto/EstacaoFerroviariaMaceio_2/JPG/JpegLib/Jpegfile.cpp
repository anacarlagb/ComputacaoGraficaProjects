...
void main()
{
	...
	UINT height;
	UINT width, m_widthDW;
	BYTE *dataBuf;

   //read the file
   dataBuf=JpegFile::JpegFileToRGB("Texture.jpg",&width, &height);
	if (dataBuf==NULL) {
		return;
	}

	// RGB -> BGR
	JpegFile::BGRFromRGB(dataBuf, width, height);
	...
	// flip that buffer
	JpegFile::VertFlipBuf(buf, m_widthDW, height);
